  ...
  return (
    <div className="p-4 py-2">
      <div className="flex items-center p-2 border rounded-xl shadow-sm">
        ...
        />
        {!isGenerating && (
          <Button className="p-2 rounded-full" variant="ghost" onClick={onSend}>
            <FaArrowUp className="h-5 w-5 text-gray-500" />
          </Button>
        )}
        {isGenerating && 
        <Button className="p-2 rounded-full" variant="ghost" onClick={onStop}>
          <FaCircleStop className="h-5 w-5" />
        </Button>}
      </div>
      <a
        href="#"
